...
export class NewsService {
  ...
  async fetchEarthquakeNews(query, pageSize = 6) {
    if (!this.apiKey) {
      throw new Error('NEWS_API_KEY is not configured');
    }

    const url = `${this.baseUrl}/everything?q=${encodeURIComponent(query)}&pageSize=${pageSize}&apiKey=${this.apiKey}&language=en&sortBy=publishedAt`;
